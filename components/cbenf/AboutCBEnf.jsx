      ...
      <div className="container mx-auto px-8 lg:px-16">
        ...
        >
          ...
          <h2 className="text-3xl md:text-4xl font-bold mb-4 text-gray-900">
            {portableTextToPlain(eventData?.about?.title) || (
          ...
          <p className="text-gray-600 max-w-3xl mx-auto">
            {portableTextToPlain(eventData?.about?.subtitle) || 'O maior evento da enfermagem brasileira está chegando a Porto Alegre'}
          </p>
        </motion.div>
        ...
        <div className="grid lg:grid-cols-2 gap-8 items-center mb-20">
          ...
          >
            ...
            <p className="text-base text-gray-700 leading-relaxed mb-4 whitespace-pre-line">
              {portableTextToPlain(eventData?.about?.description) || 'A Associação Brasileira de Enfermagem (ABEn) promove este encontro fundamental para profissionais, estudantes, pesquisadores e gestores de todo o país.\n\nUma oportunidade única de atualização científica, networking e desenvolvimento profissional em um ambiente de excelência acadêmica.'}
            </p>

            {/* Cards de Destaques */}
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-gradient-to-br from-amber-50 to-yellow-50 p-4 rounded-xl border border-amber-200">
                <div className="text-2xl font-bold text-[#D38E17] mb-1">{eventData?.about?.expectedParticipants || '+5.000'}</div>
                <p className="text-xs text-gray-600">Participantes esperados</p>
              </div>
              <div className="bg-gradient-to-br from-amber-50 to-yellow-50 p-4 rounded-xl border border-amber-200">
                <div className="text-2xl font-bold text-[#D38E17] mb-1">{eventData?.about?.edition || '75º'}</div>
          ...
          >
            <div className="relative max-w-md mx-auto">
                <Image
                  src={resolveImage(eventData?.about?.image, '/cben.webp')}
                ...
                <div className="text-xs mt-1">{eventData?.about?.parallelEvents || '7º CLAHEN • 8º SENABS'}</div>
              </div>
            </div>
          </motion.div>
        </div>
        ...
        >
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            >
              ...
              <div className="flex items-center text-gray-600 mb-4">
                ...
                <span>{eventData?.hero?.preCongressDates || '22-23 de Novembro'}</span>
              </div>
              <p className="text-sm text-gray-500">
                {eventData?.about?.preCongressDescription || 'Workshops e cursos preparatórios com especialistas renomados'}
              </p>
            </motion.div>
            ...
            >
              ...
              <div className="flex items-center mb-4">
                <Calendar className="w-4 h-4 mr-2" />
                <span>{eventData?.hero?.mainEventDates || '23-26 de Novembro'}</span>
              </div>
              <p className="text-sm text-white/90">
                {eventData?.about?.mainEventDescription || 'Palestras, painéis, apresentações de trabalhos e networking'}
              </p>
            </motion.div>
            ...
            >
              ...
              <div className="flex items-center text-gray-600 mb-4">
                ...
                <span>{eventData?.hero?.location || 'Campus PUCRS'}</span>
              </div>
              <p className="text-sm text-gray-500 whitespace-pre-line">
                {eventData?.about?.locationDescription || 'Porto Alegre - RS\nEstrutura completa e moderna'}
              </p>
            </motion.div>
          </div>
          ...
          >
            ...
            >
              ...
              <span className="relative z-10">{eventData?.about?.ctaText || 'QUERO PARTICIPAR'}</span>
