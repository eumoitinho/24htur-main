  ...
  return (
    <section className="relative min-h-screen pt-20 pb-16 overflow-hidden flex flex-col justify-center">
      ...
      <div className="absolute inset-0 z-0">
        <Image
          src={resolveImage(eventData?.hero?.backgroundImage, '/image.jpeg')}
          alt="75º CBEn"
          fill
        ...
        >
          <div className="flex items-center gap-4 mb-6">
            ...
            <div className="inline-flex items-center gap-2 bg-white/15 backdrop-blur-md border border-white/30 rounded-full py-2 px-4 shadow-lg">
              ...
              <span className="text-xs md:text-sm font-bold text-white drop-shadow-lg">{eventData?.hero?.eventName || '75º Congresso Brasileiro de Enfermagem'}</span>
            </div>
          </div>
          ...
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-extrabold mb-6 leading-tight text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.25)]">
            {portableTextToPlain(eventData?.hero?.title) || (
              <>Sua participação no{" "}
          ...
          <p className="text-lg md:text-xl text-gray-200 mb-10 max-w-3xl leading-relaxed drop-shadow-[0_1px_3px_rgba(0,0,0,0.2)]">
            {portableTextToPlain(eventData?.hero?.subtitle) || 'Esqueça o estresse de organizar hospedagem, passagens e passeios. A 24H Escritório de Viagens cuida de tudo para você focar no que realmente importa: o conhecimento e networking do maior congresso de enfermagem do Brasil.'}
          </p>
          ...
          >
            ...
            <span className="relative z-10">{eventData?.hero?.ctaText || 'QUERO MEU PACOTE EXCLUSIVO!'}</span>
        ...
        >
          ...
          >
            ...
            <p className="text-sm text-white/90 leading-relaxed drop-shadow-md font-semibold">
              {eventData?.hero?.preCongressDates && (
                <>
                  <strong>Pré-congresso:</strong> {eventData.hero.preCongressDates}<br />
                </>
              )}
              <strong>Evento:</strong> {eventData?.hero?.mainEventDates || '23-26 Nov 2025'}
            </p>
          ...
          >
            ...
            <p className="text-sm text-white/90 leading-relaxed drop-shadow-md font-semibold">
              <strong>{eventData?.hero?.location || 'Campus da PUCRS'}</strong>
            </p>
            <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-[#ae2724] to-[#DC2626] opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
          </motion.div>
          ...
          >
            ...
            <p className="text-sm text-white/90 leading-relaxed drop-shadow-md font-semibold">
              <strong>{eventData?.hero?.participants || 'Profissionais e estudantes de todo o Brasil'}</strong>
            </p>
            <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-[#ae2724] to-[#DC2626] opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
          </motion.div>
        </motion.div>
