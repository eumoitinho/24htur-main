...
const HeroTrabalheConosco = () => {
  const { data: trabalheData, loading, error } = useTrabalheConoscoPage();
  
  // Fallback para dados estáticos caso não carregue do Sanity
  const heroData = trabalheData?.hero || {
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="relative overflow-hidden rounded-[40px] sm:rounded-[44px] lg:rounded-[50px] px-6 sm:px-10 md:px-12 lg:px-16 py-12 sm:py-16 md:py-20 lg:py-24 bg-brand-dark bg-center bg-cover bg-no-repeat">
          <div className="absolute inset-0 z-0">
            <Image
              src={resolveImage(trabalheData?.hero?.backgroundImage, '/hero-trabalhe.jpg')}
              alt={trabalheData?.hero?.title || heroData?.title || 'Hero background'}
              fill
