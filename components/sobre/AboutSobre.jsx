...
const AboutSobre = () => {
  ...
  const titleText = portableTextToPlain(sobreData?.about?.title) || aboutData.title;
  const descriptionText = portableTextToPlain(sobreData?.about?.description) || aboutData.description;
  const descriptionParts = descriptionText ? descriptionText.split('\n\n') : [];
        ...
        <div className="max-w-4xl mx-auto">
          ...
          >
            {descriptionParts.length > 0 ? (
              <>
                <p className="text-lg leading-relaxed mb-6">{descriptionParts[0]}</p>
                {descriptionParts[1] && <p className="text-lg leading-relaxed">{descriptionParts[1]}</p>}
              </>
            ) : (
